--[[  Vynixius UI Library - Modificated to fit Cuen Hub's theme. 
This UI Library is not mine, credits to RegularVynixu for  Venyx and Vynixius Library.
]]

local notifLib = loadstring(game:HttpGet("https://raw.githubusercontent.com/cuenhub/Cuen_Hub/main/Utility/CuenNotif.lua"))()

local function createNotif(text, duration)
...
function Library:AddWindow(options)
	...
	function Window:AddTab(name, options)
		...
		function Tab:AddConfigs()
			...
			local Section = self:AddSection("Configuration", { default = true })

			local ConfigSection = Section:AddSubSection("Load / Save", { default = true }]
			...
			ConfigSection:AddButton("Save Config", function()
				if SaveName.Box.Text ~= "" then
                    ...
                    notifLib:CreatePromptNotif({
                      TweenSpeed = 0.5,
                       Title = "    Cuen Hub"
                       Text = "You already have a config named\n'".. fileName.."', do you wish to overwrite it?"
                       Duration = 5000
			...
			local RefreshList = ConfigSection:AddButton("Refresh List", function()
				LoadName:ClearList()
